...
export default function App() {
  ...
  const handleHapusTx = async (id) => {
    if(window.confirm('Yakin ingin menghapus transaksi ini?')) {
      ...
      showNotif('Data transaksi berhasil dihapus!'); loadData();
    }
  };
  ...
  const submitPp = async (e) => {
    ...
    showNotif('Paid Promote disimpan! Dana otomatis masuk ke Transparansi!'); loadData();
  };
    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 print:bg-white print:h-auto">
      ...
      <main className="flex-1 overflow-y-auto print:overflow-visible print:w-full">
        ...
        <div className="p-8 print:p-0">
          ...
          {/* ==========================================
              2. TRANSPARANSI FULL (FORM + TABEL)
             ========================================== */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block print:w-full">
              ...
              <div className="lg:col-span-1 print:hidden">
                <FormTransaksi theme={activeTheme} editData={editTxData} onCancelEdit={() => setEditTxData(null)}
                   onSuccess={(aksi) => { showNotif(`Transaksi manual berhasil ${aksi}!`); setEditTxData(null); loadData(); }} 
                />
              </div>
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col border border-gray-100">
                  ...
                  <div className="flex-1 p-4 overflow-x-auto">
                    <table className="w-full text-sm text-left">
                      ...
                      <tbody>
                        {paidPromotes.length === 0 ? (<tr><td colSpan="6" className="text-center py-8 text-gray-500">Belum ada PP</td></tr>) : paidPromotes.map(pp => {
                          ...
                          return (
                            <tr key={pp.id} className="border-b hover:bg-gray-50"><td className="p-3 font-mono font-bold text-gray-500">PP-{pp.id.toString().padStart(3,'0')}</td><td className="p-3 font-bold">{pp.nama_pemesan}</td><td className="p-3">{hitungDurasiHari(pp.tanggal_mulai, pp.tanggal_selesai)} Hari</td><td className="p-3 font-bold text-emerald-600">{formatRupiah(pp.harga)}</td><td className="p-3"><span className={`px-2 py-1 rounded-md text-xs font-bold ${statusVisual}`}>{statusText}</span></td><td className="p-3 text-center">{pp.status !== 'Dibatalkan' && <button onClick={()=>batalPp(pp.id)} className="text-rose-500 font-bold text-xs hover:underline">Batalkan</button>}</td></tr>
                          )
                        })}
                      </tbody>
                    </table>
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm flex flex-col border border-gray-100 overflow-hidden">
                ...
                <div className="flex-1 p-4 overflow-x-auto">
                  <table className="w-full text-sm text-left">
                      ...
                      <tbody>
                        {proposals.length === 0 ? (<tr><td colSpan="6" className="text-center py-8 text-gray-500">Belum ada proposal</td></tr>) : proposals.map(pr => (
                          <tr key={pr.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="p-3 text-center">{editPropId !== pr.id && pr.status !== 'Dicairkan' && <button onClick={()=>setEditPropId(pr.id)} className="text-blue-500 text-xs font-bold hover:underline">Ubah Status</button>}</td>
                          </tr>
                        ))}
                      </tbody>
                  </table>
