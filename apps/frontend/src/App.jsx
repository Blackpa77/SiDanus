...
const THEMES = {
  ...
  rose: {
    ...
    grad: 'from-rose-800 to-rose-600',
  },
};
...
export default function App() {
  ...
  const [inputGantiCreds, setInputGantiCreds] = useState({
  ...
  const loadData = () => {
    ...
    fetch('http://localhost:3000/api/pic')
      .then((r) => r.json())
      .then((d) => setPicList(d));
    fetch('http://localhost:3000/api/paid-promote')
      .then((r) => r.json())
      .then((d) => setPaidPromotes(d));
  ...
  // ==== CHART DATA ====
  const dataBar = [
    { name: 'Arus Kas', Pemasukan: stats.total_kredit, Pengeluaran: stats.total_debit },
  ];
  ...
  const handleLogin = (e) => {
    e.preventDefault();
    if (
      inputLogin.username === credentials.username &&
      inputLogin.password === credentials.password
    ) {
      setLoginError('');
      setIsLoggedIn(true);
  ...
  const exportExcel = () => {
    ...
    a.href = url;
    a.download = 'Laporan_Keuangan_Danus.csv';
    a.click();
  };
  ...
  const handleHapusTx = async (id) => {
    if (window.confirm('Yakin ingin menghapus transaksi ini?')) {
      ...
      showNotif('Data transaksi berhasil dihapus!');
      loadData();
    }
  };
  ...
  const submitPp = async (e) => {
    e.preventDefault();
    if (editPpData) {
      await fetch(`http://localhost:3000/api/paid-promote/${editPpData.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formPp),
      });
      showNotif('Paid Promote berhasil diupdate!');
    } else {
      await fetch('http://localhost:3000/api/paid-promote', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formPp),
      });
      showNotif('Paid Promote disimpan! Dana otomatis masuk ke Transparansi!');
    }
  ...
  const triggerEditPp = (pp) => {
    ...
    setFormPp({
      ...
      tanggal_mulai: pp.tanggal_mulai.split('T')[0],
      tanggal_selesai: pp.tanggal_selesai.split('T')[0],
      harga: pp.harga,
    });
  };
  ...
  const batalPp = async (id) => {
    if (window.confirm('Batalkan PP ini? Saldo Transparansi akan ditarik kembali.')) {
      ...
      showNotif('Paid Promote dibatalkan!');
      loadData();
    }
  };

  const hapusPp = async (id) => {
    if (window.confirm('Yakin ingin menghapus Paid Promote ini permanen? Data Transparansi terkait juga akan terhapus.')) {
      await fetch(`http://localhost:3000/api/paid-promote/${id}`, { method: 'DELETE' });
      showNotif('Paid Promote berhasil dihapus!');
      loadData();
    }
  };
  ...
  const submitProp = async (e) => {
    e.preventDefault();
    if (editPropData) {
      await fetch(`http://localhost:3000/api/proposal/${editPropData.id}/edit`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formProp),
      });
      showNotif('Data Proposal berhasil diupdate!');
    } else {
      await fetch('http://localhost:3000/api/proposal', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formProp),
      });
      showNotif('Proposal berhasil ditambah!');
    }
  ...
  const triggerEditProp = (pr) => {
    setEditPropData(pr);
    setFormProp({ pj_pengantar: pr.pj_pengantar, instansi: pr.instansi, status: pr.status });
  };

  const updateStatusProp = async (id, status, nominal) => {
    await fetch(`http://localhost:3000/api/proposal/${id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status, nominal_cair: nominal }),
    });
    setEditPropId(null);
    setEditPropNominal('');
    showNotif(status === 'Dicairkan' ? 'Hore cair! Dana otomatis masuk Transparansi!' : 'Status Proposal diupdate!');
    loadData();
  };

  const hapusProp = async (id) => {
    if (window.confirm('Yakin ingin menghapus Proposal ini permanen? Data Transparansi terkait juga akan terhapus.')) {
      await fetch(`http://localhost:3000/api/proposal/${id}`, { method: 'DELETE' });
      showNotif('Proposal berhasil dihapus!');
      loadData();
    }
  };
  ...
  const handleUpdateCreds = (e) => {
    ...
    setNotifCreds('Kredensial Login berhasil diubah.');
  ...
  const handleTambahPj = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setNotifPj(`PJ sukses ditambahkan!`);
        setNamaPjBaru('');
        loadData();
        setTimeout(() => setNotifPj(''), 4000);
      }
    } catch (err) {
      setNotifPj('Gagal nyimpan PJ!');
    }
  };
      ...
      <div className={`flex h-screen bg-gray-900 items-center justify-center font-sans`}>
        <div className="bg-white p-10 rounded-3xl shadow-2xl w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-gray-800 tracking-wider mb-2">
              Si<span className="text-emerald-500">Danus</span>
            </h1>
            <p className="text-gray-500 font-medium">Portal Keuangan Danus INFEST</p>
          </div>
          {loginError && (
            <div className="mb-4 p-3 bg-rose-50 text-rose-700 text-center rounded-lg font-bold">
              {loginError}
            </div>
          )}
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button
              type="submit"
              className="w-full bg-emerald-600 text-white font-bold py-4 rounded-xl hover:bg-emerald-700"
            >
              Masuk Dashboard
            </button>
          </form>
    ...
    <div className="flex h-screen bg-gray-50 font-sans text-gray-800 print:bg-white print:h-auto">
      ...
      <aside className={`w-64 text-white flex flex-col shadow-xl z-10 print:hidden ${activeTheme.bgSide} transition-colors duration-500`}>
        <div className={`h-20 flex items-center justify-center border-b ${activeTheme.border}`}>
          <h1 className="text-2xl font-bold tracking-wider">
            Si<span className={activeTheme.textAccent}>Danus</span>
          </h1>
        </div>
        <nav className="flex-1 px-4 py-6 space-y-2">
          {['dashboard', 'transparansi', 'paid promote', 'proposal', 'pengaturan'].map((menu) => (
            <button
              ...
              className={`w-full text-left px-4 py-3 rounded-lg font-semibold uppercase text-sm transition ${
                activeMenu === menu ? 'bg-white/20 shadow-sm' : 'hover:bg-white/10'
              }`}
            >
              {menu}
            </button>
        ...
        <div className={`p-4 border-t ${activeTheme.border}`}>
          <button
            ...
            className="w-full bg-rose-600 hover:bg-rose-700 text-white font-bold py-3 rounded-lg"
          >
            Logout
          </button>
        </div>
      ...
      <main className="flex-1 overflow-y-auto print:overflow-visible print:w-full">
        <header className="bg-white shadow-sm h-20 flex items-center px-8 sticky top-0 z-10 print:hidden justify-between">
          ...
          <div className="font-bold text-gray-500 uppercase">Divisi Danus INFEST</div>
        </header>

        <div className="p-8 print:p-0">
          {/* NOTIFIKASI GLOBAL */}
          {notifTable && (
            <div className="mb-6 p-4 bg-emerald-100 text-emerald-800 font-bold rounded-xl text-center shadow-sm print:hidden border border-emerald-200">
              {notifTable}
            </div>
          )}
          ...
          {activeMenu === 'dashboard' && (
            <div className="space-y-6 print:hidden">
              <div className={`bg-gradient-to-r text-white p-6 rounded-2xl shadow-md flex justify-between items-center ${activeTheme.grad} transition-colors duration-500`}>
                <div>
                  <h3 className="text-sm font-medium mb-1">Waktu Saat Ini</h3>
                  <p className="text-xl font-bold">
                    {waktuLive.toLocaleDateString('id-ID', {
                      weekday: 'long',
                      year: 'numeric',
                      month: 'long',
                      day: 'numeric',
                ...
                <div className="text-3xl font-mono font-bold bg-black/20 px-6 py-3 rounded-xl shadow-inner tracking-widest">
                  {waktuLive.toLocaleTimeString('id-ID', {
                    hour: '2-digit',
                    minute: '2-digit',
                    second: '2-digit',
                  })}{' '}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-blue-500">
                  <p className="text-sm text-gray-500 font-semibold mb-1">Total Saldo Danus</p>
                  <h3 className="text-3xl font-bold">{formatRupiah(stats.saldo_saat_ini)}</h3>
                </div>
                <div className="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-emerald-500">
                  <p className="text-sm text-gray-500 font-semibold mb-1">Pemasukan (+)</p>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm border-l-4 border-rose-500">
                  <p className="text-sm text-gray-500 font-semibold mb-1">Pengeluaran (-)</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block print:w-full">
              <div className="lg:col-span-1 print:hidden">
                <FormTransaksi
                  theme={activeTheme}
                  editData={editTxData}
                  onCancelEdit={() => setEditTxData(null)}
                  onSuccess={(aksi) => {
                    showNotif(`Transaksi manual berhasil ${aksi}!`);
                    setEditTxData(null);
                    loadData();
                  }}
                />
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100 flex flex-col print:shadow-none print:border-0 print:m-0 print:p-0 print:block">
                <div className="px-6 py-4 border-b bg-gray-50 flex justify-between items-center print:hidden">
                  <h4 className="font-bold text-gray-700">Riwayat Transaksi Utama</h4>
                  <div className="flex gap-2">
                    <button onClick={exportPDF} className="bg-rose-500 hover:bg-rose-600 text-white text-sm font-bold py-2 px-4 rounded-lg">
                      Ekspor PDF
                    </button>
                    <button onClick={exportExcel} className="bg-green-600 hover:bg-green-700 text-white text-sm font-bold py-2 px-4 rounded-lg">
                      Ekspor Excel
                    </button>
                  </div>
                </div>
                ...
                <div className="hidden print:block text-center pt-4 pb-6 w-full">
                  ...
                  <h3 className="text-lg font-bold text-gray-700 uppercase">Divisi Danus - INFEST 2026</h3>
                  <p className="text-sm text-gray-500 mt-1">
                    Dicetak pada: {formatTanggalDB(new Date().toISOString())}
                  </p>
                ...
                <div className="flex-1 p-4 print:p-0 overflow-x-auto print:overflow-visible">
                  <table className="w-full text-sm text-left text-gray-600 print:border-collapse print:w-full print:border print:border-gray-800">
                    ...
                    <tbody>
                      {transaksi.length === 0 ? (
                        <tr>
                          <td colSpan="7" className="text-center py-8 print:border print:border-gray-800">
                            Belum ada transaksi
                          </td>
                        </tr>
                      ) : (
                        transaksi.map((tx) => (
                          <tr key={tx.id} className="border-b hover:bg-gray-50 print:border-b print:border-gray-800">
                            <td className="px-4 py-3 font-mono font-bold text-gray-500 print:border print:border-gray-800">
                              TRX-{tx.id.toString().padStart(4, '0')}
                            </td>
                            <td className="px-4 py-3 whitespace-nowrap print:border print:border-gray-800">
                              {formatTanggalDB(tx.tanggal)}
                            </td>
                            <td className="px-4 py-3 font-medium text-gray-800 print:border print:border-gray-800">
                              {tx.keterangan || '-'}
                            </td>
                            ...
                            <td
                              className={`px-4 py-3 font-bold whitespace-nowrap text-right print:border print:border-gray-800 ${
                                tx.tipe === 'KREDIT' ? 'text-emerald-600 print:text-gray-900' : 'text-rose-600 print:text-gray-900'
                              }`}
                            >
                              {tx.tipe === 'KREDIT' ? '+' : '-'}
                            ...
                            <td className="px-4 py-3 text-center print:hidden whitespace-nowrap">
                              <button onClick={() => setEditTxData(tx)} className="text-blue-500 hover:text-blue-700 font-bold mx-2">
                                Edit
                              </button>
                              <button onClick={() => handleHapusTx(tx.id)} className="text-rose-500 hover:text-rose-700 font-bold mx-2">
                                Hapus
                              </button>
                            </td>
                          </tr>
                    ...
                    <tfoot className="hidden print:table-footer-group font-bold text-gray-900 bg-gray-100">
                      <tr>
                        <td colSpan="5" className="px-4 py-3 text-right print:border print:border-gray-800">
                          TOTAL PEMASUKAN
                        </td>
                        <td className="px-4 py-3 text-right print:border print:border-gray-800">
                          {formatRupiah(stats.total_kredit)}
                        </td>
                      </tr>
                      <tr>
                        <td colSpan="5" className="px-4 py-3 text-right print:border print:border-gray-800">
                        ...
                        <td className="px-4 py-3 text-right print:border print:border-gray-800">
                          {formatRupiah(stats.total_debit)}
                        </td>
                      </tr>
                      <tr>
                        <td colSpan="5" className="px-4 py-3 text-right print:border print:border-gray-800 uppercase">
                          Saldo Akhir
                        </td>
                        <td className="px-4 py-3 text-right print:border print:border-gray-800 text-lg">
                          {formatRupiah(stats.saldo_saat_ini)}
                        </td>
                      </tr>
                    </tfoot>
                  </table>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm h-fit border border-gray-100">
                ...
                <form onSubmit={submitPp} className="space-y-4">
                  <div>
                    <label className="text-sm font-medium mb-1 block">Nama Pemesan</label>
                    <input
                      type="text"
                      value={formPp.nama_pemesan}
                      onChange={(e) => setFormPp({ ...formPp, nama_pemesan: e.target.value })}
                      required
                      className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                    />
                  </div>
                  <div className="flex gap-2">
                    <div className="flex-1">
                      ...
                      <input
                        type="date"
                        value={formPp.tanggal_mulai}
                        onChange={(e) => setFormPp({ ...formPp, tanggal_mulai: e.target.value })}
                        required
                        className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                      />
                    </div>
                    <div className="flex-1">
                      ...
                      <input
                        type="date"
                        value={formPp.tanggal_selesai}
                        onChange={(e) => setFormPp({ ...formPp, tanggal_selesai: e.target.value })}
                        required
                        className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                      />
                    </div>
                  </div>
                  <div className="text-sm text-gray-500 font-bold text-right">
                    Durasi: {hitungDurasiHari(formPp.tanggal_mulai, formPp.tanggal_selesai)} Hari
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      value={formPp.harga}
                      onChange={(e) => setFormPp({ ...formPp, harga: e.target.value })}
                      required
                      className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                    />
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      ...
                      className={`flex-1 text-white font-bold py-3 rounded-lg ${
                        editPpData ? 'bg-orange-500 hover:bg-orange-600' : activeTheme.btn
                      }`}
                    >
                      {editPpData ? 'Update PP' : 'Simpan PP'}
                    </button>
                    {editPpData && (
                      <button
                        type="button"
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm overflow-hidden flex flex-col border border-gray-100">
                <div className="px-6 py-4 border-b bg-gray-50">
                  <h4 className="font-bold text-gray-700">Riwayat Paid Promote</h4>
                </div>
                          ...
                          return (
                            <tr key={pp.id} className="border-b hover:bg-gray-50">
                              <td className="p-3 font-mono font-bold text-gray-500">
                                PP-{pp.id.toString().padStart(3, '0')}
                              </td>
                              <td className="p-3 font-bold">{pp.nama_pemesan}</td>
                              <td className="p-3">
                                {hitungDurasiHari(pp.tanggal_mulai, pp.tanggal_selesai)} Hari
                              </td>
                              <td className="p-3 font-bold text-emerald-600">{formatRupiah(pp.harga)}</td>
                              <td className="p-3">
                                <span className={`px-2 py-1 rounded-md text-xs font-bold ${statusVisual}`}>
                                  {statusText}
                                </span>
                              </td>
                              <td className="p-3 text-center whitespace-nowrap">
                                {pp.status !== 'Dibatalkan' && (
                                  <button
                                    onClick={() => batalPp(pp.id)}
                                    className="text-blue-500 font-bold text-xs hover:underline mx-1"
                                  >
                                    Batalkan
                                  </button>
                                )}
                                <button
                                  onClick={() => triggerEditPp(pp)}
                                  className="text-orange-500 text-xs font-bold hover:underline mx-1"
                                >
                                  Edit
                                </button>
                                <button
                                  onClick={() => hapusPp(pp.id)}
                                  className="text-rose-500 text-xs font-bold hover:underline mx-1"
                                >
                                  Hapus
                                </button>
                              </td>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1 bg-white p-6 rounded-2xl shadow-sm h-fit border border-gray-100">
                ...
                <form onSubmit={submitProp} className="space-y-4">
                  <div>
                    <label className="text-sm font-medium mb-1 block">PJ Pengantar (Manual)</label>
                    <input
                      type="text"
                      value={formProp.pj_pengantar}
                      onChange={(e) => setFormProp({ ...formProp, pj_pengantar: e.target.value })}
                      required
                      className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                    />
                  </div>
                  <div>
                    <label className="text-sm font-medium mb-1 block">Instansi Tujuan</label>
                    <input
                      type="text"
                      value={formProp.instansi}
                      onChange={(e) => setFormProp({ ...formProp, instansi: e.target.value })}
                      required
                      className={`w-full p-2 border rounded-lg ${activeTheme.ring}`}
                    />
                  </div>
                  ...
                  <div className="flex gap-2">
                    <button
                      type="submit"
                      className={`flex-1 text-white font-bold py-3 rounded-lg ${
                        editPropData ? 'bg-orange-500 hover:bg-orange-600' : activeTheme.btn
                      }`}
                    >
                      {editPropData ? 'Update Proposal' : 'Simpan Proposal'}
                    </button>
                    {editPropData && (
                      <button
                        type="button"
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm flex flex-col border border-gray-100 overflow-hidden">
                <div className="px-6 py-4 border-b bg-gray-50">
                  <h4 className="font-bold text-gray-700">Riwayat Proposal</h4>
                </div>
                <div className="flex-1 p-4 overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    <thead className="bg-gray-100 text-xs uppercase">
                      <tr>
                        <th className="p-3">ID</th>
                        <th className="p-3">PJ Pengantar</th>
                        <th className="p-3">Instansi</th>
                        <th className="p-3">Status</th>
                        <th className="p-3">Cair</th>
                        <th className="p-3 text-center">Aksi</th>
                      </tr>
                    </thead>
                    <tbody>
                      {proposals.length === 0 ? (
                        ...
                        proposals.map((pr) => (
                          <tr key={pr.id} className="border-b hover:bg-gray-50">
                            <td className="p-3 font-mono font-bold text-gray-500">
                              PROP-{pr.id.toString().padStart(3, '0')}
                            </td>
                            <td className="p-3">{pr.pj_pengantar}</td>
                            <td className="p-3 font-bold">{pr.instansi}</td>
                            <td className="p-3">
                              {editPropId === pr.id ? (
                                <select
                                  value={pr.status}
                                  onChange={(e) => {
                                    if (e.target.value !== 'Dicairkan')
                                      updateStatusProp(pr.id, e.target.value, null);
                                    else setEditPropId(pr.id);
                                  }}
                                  className="p-2 border rounded-md text-xs font-bold shadow-sm"
                                >
                              ...
                              {editPropId === pr.id ? (
                                <div className="flex gap-2 items-center">
                                  <input
                                    ...
                                    value={editPropNominal}
                                    onChange={(e) => setEditPropNominal(e.target.value)}
                                    className="w-24 p-1 text-xs border rounded-md"
                                  />
                                  <button
                                    onClick={() => updateStatusProp(pr.id, 'Dicairkan', editPropNominal)}
                                    className="bg-emerald-500 text-white px-3 py-1 text-xs rounded-md font-bold"
                                  >
                                    OK
                                  </button>
                                </div>
                                ...
                                <span className="font-bold text-emerald-600">
                                  {pr.nominal_cair ? formatRupiah(pr.nominal_cair) : '-'}
                                </span>
                              )}
                            </td>
                            <td className="p-3 text-center whitespace-nowrap">
                              {editPropId === pr.id ? (
                                <button
                                  ...
                                  className="text-gray-500 text-xs font-bold hover:underline mx-1"
                                >
                                  Batal
                                </button>
                              ) : (
                                pr.status !== 'Dicairkan' && (
                                  <button
                                    onClick={() => setEditPropId(pr.id)}
                                    className="text-blue-500 text-xs font-bold hover:underline mx-1"
                                  >
                                    Status
                                  </button>
                                )
                              ...
                              <button
                                onClick={() => triggerEditProp(pr)}
                                className="text-orange-500 text-xs font-bold hover:underline mx-1"
                              >
                                Edit
                              </button>
                              <button
                                onClick={() => hapusProp(pr.id)}
                                className="text-rose-500 text-xs font-bold hover:underline mx-1"
                              >
                                Hapus
                              </button>
                  ...
                  <div className="flex gap-4">
                    {Object.keys(THEMES).map((key) => (
                      <button
                        ...
                        onClick={() => {
                          setThemeKey(key);
                          localStorage.setItem('sidanus_theme', key);
                        }}
                        className={`flex-1 py-3 px-4 rounded-xl font-bold text-white transition ${
                          THEMES[key].btn
                        } ${themeKey === key ? 'ring-4 ring-offset-2 ring-gray-300' : ''}`}
                      >
                        {THEMES[key].name}
                      </button>
                    ))}
                  ...
                  <form onSubmit={handleUpdateCreds} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        required
                        className={`w-full border-gray-300 rounded-lg p-3 border ${activeTheme.ring}`}
                      />
                    </div>
                    <div>
                    ...
                    <button type="submit" className={`w-full text-white font-bold py-3 rounded-lg ${activeTheme.btn}`}>
                      Simpan Kredensial
                    </button>
                  </form>
                </div>
              ...
              <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 h-fit">
                ...
                <form onSubmit={handleTambahPj} className="flex gap-4 mb-6">
                  <input
                    ...
                    placeholder="Nama PJ (cth: Dika)"
                    required
                    className={`flex-1 border-gray-300 rounded-lg p-3 border ${activeTheme.ring}`}
                  />
                  <button type="submit" className={`text-white font-bold py-3 px-6 rounded-lg ${activeTheme.btn}`}>
                    Tambah
                  </button>
                </form>
                  ...
                  <ul className="space-y-2 max-h-60 overflow-y-auto">
                    {picList.map((pj) => (
                      ...
                      >
                        ...
                        <button
                          onClick={() => handleHapusPj(pj.id, pj.nama)}
                          className="text-rose-500 hover:bg-rose-100 text-sm font-bold px-3 py-1 rounded-md transition"
                        >
                          Hapus
                        </button>
                      </li>
